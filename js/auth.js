...
window.YYCardAuth = {
    ...
    async init() {
        ...
        if (!user) {
            if (isTelegram) {
                this.log('会话无效，重新通过 Telegram 认证...', true);
                try {
                    const res = await fetch(`${window.YYCardConfig.SUPABASE_URL}/functions/v1/telegram-simple-auth`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                    ...
                    const body = await res.json();
                    if (body.error) {
                        this.log(`重新认证失败: ${body.error}`, true);
                        // 清除标记，避免死循环
                    ...
                    this.log(`重新认证异常: ${err.message}`, true);
                    sessionStorage.removeItem('yy_logged_in');
                    window.location.href = window.YYCardConfig.LOGIN_PAGE_URL;
                    return false;
                }
